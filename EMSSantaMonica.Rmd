...
ggplot() + #AD - used this to diagnose hte issue with the missing data/geometries
  geom_sf(data = vars_net_tractData, colour = "red", fill = "transparent") +
  geom_sf(data = tractData, colour = "blue", fill = "transparent") 
...
EMS_2020 <- subset(EMS_int,
     ...
     incident_date <= as.POSIXct('2020-12-09')
     ) 
#make SF
...
EMS2020.sf <-
  EMS2020.sf %>%
  mutate(CoastDist=(geosphere::dist2Line(p=st_coordinates(st_centroid(EMS2020.sf)),
                                        line=st_coordinates(coastline$osm_lines)[,1:2])*0.00062137)[,1],
         distWater = housesOSM$CoastDist,
...
EMS2020.sf <-
  EMS2020.sf %>%
    mutate(
      bar.nn =
        nn_function(st_c(st_coid(EMS2020.sf)), st_c(bars),1),
      sport.nn =
        nn_function(st_c(st_coid(EMS2020.sf)), st_c(st_coid(sport)),1),
      downtown.nn =
        nn_function(st_c(st_coid(EMS2020.sf)), st_c(st_coid(downtown)),3),
      intersection.nn = 
        nn_function(st_c(st_coid(EMS2020.sf)), st_c(intersections),3))
...
EMS2020_net.response <- 
  dplyr::select(EMS2020.sf) %>% 
  mutate(responseTime = na.omit(EMS2020.sf$interval)) %>% 
  aggregate(., fishnet, mean) %>%
  mutate(responseTime = replace_na(responseTime, 0),
         uniqueID = as.numeric(rownames(.)),
         cvID = sample(round(nrow(fishnet) / 24), size=nrow(fishnet), replace = TRUE))

EMS2020_net <- full_join(EMS2020_net.count, st_drop_geometry(EMS2020_net.response), by = c("uniqueID"))

final_net <-
  inner_join(EMS2020_net, st_drop_geometry(vars_net_tractData), by="uniqueID")
...
ggplot() +
  geom_sf(data = EMS2020_net, aes(fill = countEMS, colour = countEMS)) +
...
ggplot() +
  ...
  scale_fill_viridis(option = "A", name = "Incidents Per Cell") +
  scale_colour_viridis(option = "A", name = "Incidents Per Cell") +
  labs(title = "Observed EMS Calls Joined to Fishnet, 2010-2020", subtitle = "Santa Monica, CA") +
  mapTheme()


#Map response time 2020
  ggplot() +
  geom_sf(data = EMS2020_net.response, aes(fill = responseTime, colour = responseTime)) +
...
#Map response time since 2010
  ggplot() +
  geom_sf(data = EMS_net.emg, aes(fill = responseTime, colour = responseTime)) +
  scale_fill_viridis(option = "A", name = "Minutes") +
  scale_colour_viridis(option = "A", name = "Minutes") +
...
dat_net <- st_join(EMS_2020,fishnet, join=st_intersects,left = TRUE) %>%
  mutate(longitude = unlist(map(geometry, 1)),
         latitude = unlist(map(geometry, 2)))%>%
  as.data.frame() 

dat_net.2010 <- st_join(EMS_2010,fishnet, join=st_intersects,left = TRUE) %>%
  mutate(longitude = unlist(map(geometry, 1)),
         latitude = unlist(map(geometry, 2)))%>%
  as.data.frame() 
...
grid.arrange(
  ...
  ggplot(weather.Panel, aes(interval60,visibility)) + geom_line() + 
    labs(title="Temperature", x="Month", y="visibility") + plotTheme(),
  ggplot(weather.Panel, aes(interval60,wind_gust)) + geom_line() + 
    labs(title="Temperature", x="Month", y="wind gust") + plotTheme(),
...
#haven't tried
set.seed(10)
trainIndex <- createDataPartition(final_net$count_EMS, 
                                  p = .65,
                                  list = FALSE,
                                  times = 1)
EMSTrain <- housingsubsidy[ trainIndex,]
EMSTest  <- housingsubsidy[-trainIndex,]

reg0 <- glm(CountEMS ~ .,
                  data=EMSTrain %>% 
                    dplyr::select(-CountEMS), #delete all the NAs
                  family="binomial" (link="logit"))

testProbs_ks <- data.frame(Outcome = as.factor(EMSTestt$Count_EMS),
