...
2. Service requests, trash complains, violent crime, binge drinking, and bars are clustered in the downtown region, the biggest hotspot for EMS calls.  
...
To verify these correlations with EMS calls, we plot EMS calls as a function of these risk factors. The results verify our suspicions: the most correlated spatial risk factors are violent crime, 311 service requests, downtown, trash complaints, distance to nearest bar, binge ranking, median income, and poverty.
...
covid <- st_read("/Users/annaduan/Documents/GitHub/EMS-Prediction/coronavirus.csv") %>%
  ...
  dplyr::select(date, confirmed_cases, new_cases, geometry) %>%
...
reg_temp1 <- 
  lm(EMS_Count ~  reporting_district + hour(interval60) + dotw + Temperature + Precipitation + visibility + wind_gust + Wind_Speed + lagHour + lag2Hours +lag3Hours +lag12Hours + lag4Hours + lag1day, data = EMSallTrain)
...
week_predictions <- 
  EMS.Test.weekNest %>% 
    mutate(Time_Space_FE_timeLags = map(.x = data, fit = reg_temp1, .f = model_pred)) %>%
