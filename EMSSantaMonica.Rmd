...
resp_1819 <-
  ggplot() +
  geom_sf(data = EMS_18_19_net.response, aes(fill = responseTime, colour = responseTime)) +
  scale_fill_viridis(option = "A", name = "Minutes") +
  scale_colour_viridis(option = "A", name = "Minutes") +
  labs(title = "2018-2019", subtitle = "Santa Monica, CA") +
  mapTheme()
...
p3 <- grid.arrange(arrangeGrob(resp_2020 + theme(legend.position="none"),
                               resp_1819 + theme(legend.position="none"),
                               nrow=1),
...
study.panel.rd <- 
  EMS_18_20 %>%
  mutate(EMS_Counter = 1) %>%
  right_join(st_drop_geometry(EMS_18_20)) %>% 
...
tg   <- as.POSIXct("2020-11-22 01:00:00 UTC")
