...
study.panel.net <- dat_net %>%
  dplyr::select(reporting_district, station, interval15, tract, servicerequest, Trash, Violent_Crime, ZIPCODE, EMS.isSig.dist, uniqueID) %>%
  merge(x=study.panel, y=., by.x = "uniqueID", by.y = "uniqueID", all.x = TRUE) %>%
      filter(is.na(uniqueID) == FALSE)
