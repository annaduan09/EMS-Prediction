...
p3 <- grid.arrange(arrangeGrob(resp_2020 + theme(legend.position="none"),
                               resp_1820 + theme(legend.position="none"),
                               nrow=1),
...
trainA <- c(1, 2, 3, 4, 5,3,2,1)
trainB <- c(6,7,8,9,10,34,56,78)
trainC <- c(34,56,78,90,87,65,54,43)

trainD <- c(9,8,7,6,5,1,2,3)
trainE <- c(5,6,7,8,9,4,6,2)


test.train <- data.frame(trainA, trainB, trainC, trainD, trainE)
test.test <- data.frame(trainA, trainB, trainC, trainD, trainE)

reg_test <- 
  lm(trainA ~  trainB + trainC + trainD + trainE,
     data=test.train)
...
trainB_predictions <- 
  test.test.trainBNest %>% 
    mutate(Time_Space_FE_timeLags = map(.x = data, fit = reg_test, .f = model_pred)) %>% 
    gather(Regression, Prediction, -data, -trainB) %>%
    mutate(Observed = map(data, pull, trainA),
...
week_predictions <- 
  EMS.Test.weekNest %>% 
    mutate(Time_Space_FE_timeLags = map(.x = data, fit = reg_temp, .f = model_pred)) %>% 
