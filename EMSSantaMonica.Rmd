...
```{r calls / time of day, fig.height=4, fig.width=10, message=FALSE, warning=FALSE, results='hide'}
grid.arrange(
...
```{r weather conditions/time plot, fig.height=10, fig.width=6, message=FALSE, warning=FALSE, results='hide'}
library(ggplot2)
library(gridExtra)
#vsby: visibility; sknt:wind speed; gust:wind gust in knots
grid.arrange(
...
################Holidays################
mondays <- #for reference in the graphic
  mutate(study.panel,
...
```{r Split data, message=FALSE, warning=FALSE, results='hide'}
...
reg1_time <- 
  lm(EMS_Count ~  hour(interval60) + dotw + Temperature + Precipitation + visibility + wind_gust + Wind_Speed + isWeekend + isHoliday + isPeakHour, data = EMS.Train)
...
reg4_lag <-   lm(EMS_Count ~ hour(interval60) + dotw + Temperature + Precipitation + visibility + wind_gust + Wind_Speed + +isWeekend + isHoliday + isPeakHour + uniqueID + Violent_Crime + bar.nn + downtown.nn + lagHour + lag2Hours + lag4Hours + lag12Hours + lag1day, data = EMS.Train)
...
week_predictions <- 
  EMS.Test.weekNest %>% 
    ...
    gather(Regression, Prediction, -data, -Week) %>%  # should gather by cell & week - find uniqueID
...
#select only uniqueID of one, join by incident, not reporting district
...
ggplot()+
  ...
  geom_point(data = week_predictions.twocol, aes(x = longitude, y = latitude, color = MAE), 
...
ggplot(week_predictions.twocol, aes(reporting_district, MAE)) +  #AD: not working - empty MAE
