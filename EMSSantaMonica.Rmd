...
# Data Exploration
To begin our analysis, we wrangle EMS call data to understand the spatial trends in EMS call and response over time.
...
EMS_18_19 <- subset(EMS,
     incident_date >= as.POSIXct('2018-12-08') &
     incident_date <= as.POSIXct('2019-12-08')
     )
#SF
...
final_net_viz <- inner_join(EMS_18_19_net, st_drop_geometry(vars_net_tractData), by="uniqueID")
...
mylegend<-g_legend(resp_2020)

p3 <- grid.arrange(arrangeGrob(resp_2020 + theme(legend.position="none"),
                               resp_1020 + theme(legend.position="none"),
                               nrow=1),
...
```{r risk factor scatterplots, fig.height=16, fig.width=12, message=FALSE, warning=FALSE, results='hide'}
...
What about the temporal risk factors for EMS incidents? First, we visualize the incidence of EMS calls over the past year. It appears that calls peak in spring, drop during the summer, and pick up again in fall. Figure XX then shows a two-week range in April, 2019. Here, we see that demand is highest at the start of both weeks, and drops for the rest of the week and the weekend.
...
# hourly - can see a peak on Monday both weeks
ggplot(EMS_18_20 %>%
         filter(interval60 >= as.POSIXct('2019-4-1') &
     interval60 <= as.POSIXct('2019-4-14')
     ) %>%
         group_by(interval60) %>%
         tally())+
  geom_line(aes(x = interval60, y = n))+
  labs(title="EMS Calls per hr,\n Santa Monica, April 2019",
       x="Week", 
       y="Number of trips")+
  plotTheme()
```

Next, we examine in greater detail when EMS calls occur. Figure XX shows that they are most frequent on weekdays, and that it first peaks at 1am, then at 1pm and 3pm respectively on weekdays and weekends. Additionally, cases peak on Monday.
```{r calls / time of day, fig.height=10, fig.width=18, message=FALSE, warning=FALSE, results='hide'}
grid.arrange(
...
#origin map      #AD: why is this map rotated 90 degrees?
ggplot()+
...
grid.arrange(
ggplot(covid, aes(date,new_cases)) + 
  geom_area(fill = "pink") +
  geom_line(color = "pink") +
  labs(title = "Covid-19 Cases, 2020", subtitle = "Santa Monica, CA", x="Date", y="Case Count") + 
  plotTheme(),
ggplot(covid, aes(date,two_week_lag)) + 
  geom_area(fill = "pink") +
  geom_line(color = "pink") +
  labs(title = "Covid-19 14-day Lag, 2020", subtitle = "Santa Monica, CA", x="Date", y="Cases from 2 weeks ago") + 
  plotTheme(),
ncol = 2,
top = "Covid-19 Cases and 14-day Lag, 2020"
)
```
...
```{r weather conditions/time plot, fig.height=20, fig.width=8, message=FALSE, warning=FALSE, results='hide'}
library(ggplot2)
library(gridExtra)
#vsby: visibility; sknt:wind speed; gust:wind gust in knots
grid.arrange(
...
```{r EMS calls / Holidays, message=FALSE, warning=FALSE, include=TRUE, results='hide'}
library(dplyr)
ACSTracts <- ACSTracts %>%
...
#STUDY.PANEL
study.panel <- 
  expand.grid(interval60=unique(dat_net$interval60), #it's because we don't have interval 60 here
              uniqueID = unique(dat_net$uniqueID))
EMS_18_20.sample <- EMS_18_20[sample(nrow(EMS_18_20), 1500), ]
study.panel.rd <-
  expand.grid(interval60=unique(EMS_18_20$interval60), 
              reporting_district= unique(EMS_18_20$reporting_district))
study.panel.15 <- 
...
study.panel.rd <- 
  EMS_18_20 %>%
  mutate(EMS_Counter = 1) %>%
  right_join(st_drop_geometry(EMS_18_20.sf)) %>% 
...
```{r Maps/plot: Local Morans: Identify hotspots + add isSig features, message=FALSE, warning=FALSE, results='hide'}
...
corlag.emsct <- na.omit(study.panel.rd[c(-1), c("interval60","EMS_Count")])%>%   
  ...
  subset(.,
     interval60 >= as.POSIXct('2019-11-10 01:00:00 UTC') &
     interval60 <= as.POSIXct('2019-12-29 01:00:00 UTC')
     )
...
#    group_by(interval4h, reporting_district) %>%
...
```{r temporal model, message=FALSE, warning=FALSE, results='hide'}

#split data sets
library(caret)
set.seed(3456)
trainIndex <- createDataPartition(study.panel.rd$EMS_Count, 
                                  y = paste(study.panel.rd$reporting_district),
                                  p = .65,
                                  list = FALSE,
                                  times = 1)
...
study.panel.net <- left_join(study.panel, panel_net, by= "uniqueID") %>%
  na.omit() %>%
  mutate(tract = as.factor(tract))

#test corelation
net.sample <- study.panel.net %>%     #AD: study.panel.net was way too big for my computer to make the corrplot haha
  sample_n(., 20000)

numericVars <-
  select_if(net.sample, is.numeric) %>% na.omit()
...
#EMSallTrain <- study.panel.net[ trainIndex,]
#EMSallTest  <- study.panel.net[-trainIndex,] 
...
#EMSallTrain <- EMSallTrain %>%
#  rename(Week = week)

#EMSallTest <- EMSallTest %>%
#  rename(Week = week) 
...
reg1_time <- 
  lm(EMS_Count ~  hour(interval60) + dotw + Temperature + Precipitation + visibility + wind_gust + Wind_Speed, data = EMS.Train)

reg2_space <- 
  lm(EMS_Count ~ uniqueID + Violent_Crime + bar.nn + downtown.nn, data = EMS.Train)
  
reg3_time_space <- 
  lm(EMS_Count ~ hour(interval60) + dotw + Temperature + Precipitation + visibility + wind_gust + Wind_Speed + uniqueID + Violent_Crime + bar.nn + downtown.nn, data = EMS.Train)

reg4_lag <-   lm(EMS_Count ~ hour(interval60) + dotw + Temperature + Precipitation + visibility + wind_gust + Wind_Speed + uniqueID + Violent_Crime + bar.nn + downtown.nn + lagHour + lag2Hours + lag4Hours + lag12Hours + lag1day, data = EMS.Train)
...
#PLOT AGAINST ACTUAL SERIES - AD: I got this to work!
week_predictions %>% 
...
week_predictions.rd <- week_predictions %>%
  mutate(interval60 = map(data, pull, interval60)) %>%
  dplyr::select(interval60, Observed, Prediction, Regression, Absolute_Error, sd_AE, MAE) %>%
  unnest() %>%
  left_join(., EMS_18_20, by="interval60")


week_predictions.rd_sample <- week_predictions.rd %>%
  sample_n(., 40000)


#AD: this is as far as I got.. my computer's ram is too low even though I made samples of everything..

week_predictions.twocol <-week_predictions.rd_sample %>% #AD: running out of vector memory - not sure how to make this smaller
  left_join(., EMS_2020[,c("reporting_district", "latitude", "longitude")],by = "reporting_district") %>%
  filter(Regression == "Time_Space_FE_timeLags") %>% ##regresstion
  group_by(reporting_district) %>%
  summarize(MAE = mean(abs(Observed-Prediction), na.rm = TRUE))

week_predictions.rd_sample <- left_join(week_predictions.rd_sample, week_predictions.twocol, by="reporting_district")

week_predictions.latlon <- left_join(week_predictions.rd_sample, EMS_2020[,c("reporting_district", "latitude", "longitude", "dotw", "week", "interval60")],by = "reporting_district")

week_predictions.latlon_sample <- week_predictions.latlon %>%
  sample_n(., 40000)

ggplot()+
  geom_sf(data = SM_Base, color = "grey", fill = "transparent") +
  geom_point(data = week_predictions.latlon_sample, aes(x = longitude.x, y = latitude.x, color = MAE), 
...
ggplot() +
  geom_sf(data = SM_Base) +
  mapTheme()

#ggplot(dweek_predictions.twocol, aes(reporting_district, MAE)) +
...
error.byWeek <-
  week_predictions.latlon%>%
  dplyr::select(reporting_district, Absolute_Error, Week) %>%
  gather(Variable, Value, -reporting_district, -Week) %>%
    group_by(Variable, reporting_district, Week) %>%
    summarize(MAE = mean(Value))

error.byHour <-
    week_predictions.latlon%>%
    dplyr::select(reporting_district, Absolute_Error, interval60.x) %>%
    gather(Variable, Value, -interval60.x, -reporting_district) %>%
    filter(wday(interval60.x, label = TRUE) == "Mon" & week(interval60.x) == 50) %>%
    group_by(hour = hour(interval60.x), reporting_district) %>%
    summarize(MAE = mean(Value)) 
...
EMSsample <- sample_n(study.panel.net, 100000)%>%
  na.omit()
...
ggplot(reg.cv.k$resample, aes(x=MAE)) +
  ...
  labs(title = "Figure 6.1: Mean Average Error in Cross Validation Tests with model ks") 

ggplot(reg.cv.k.health$resample, aes(x=MAE)) +
  geom_histogram(bins = 30, colour="black", fill = "#FDE725FF") +
  labs(title = "Figure 6.2: Mean Average Error in Cross Validation Tests with model ks plus Addtional health Features") 

ggplot(reg.cv.k.space$resample, aes(x=MAE)) +
  geom_histogram(bins = 30, colour="black", fill = "#FDE725FF") +
  labs(title = "Figure 6.3: Mean Average Error in Cross Validation Tests with model 4 plus Addtional health and spatial Features") 
...
```{r plot: predicted vs observed, message=FALSE, warning=FALSE, results='hide'}
#Error by fishnet cell
week_predictions %>% 
  mutate(interval60 = map(data, pull, interval60),
         uniqueID = map(data, pull, uniqueID), 
         uniqueID = map(data, pull, uniqueID), 
         uniqueID = map(data, pull, start_station_longitude)) %>%
...
```{r map: spatial distribution of MAE, message=FALSE, warning=FALSE, results='hide'}
week_predictions %>% 
    mutate(interval60 = map(data, pull, interval60),
           start_station = map(data, pull, start_station), 
           start_lat = map(data, pull, start_lat), 
