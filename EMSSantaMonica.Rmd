...
```{r risk factor maps, fig.height=12, fig.width=15}
#select features
...
We also look to holidays. On these days, there is expected to be a higher density of people outdoors celebrating and likely more injuries as well. To do this, we identify a few of the holidays where Americans consume the most alcohol, and plot them against EMS calls. From Figure XX, it appears that ___, ___, and ____ do align with peaks in demand.
```{r EMS calls / Holidays}
library(dplyr)
ACSTracts <- ACSTracts %>%
...
# Study Panels  
The next step in this analysis is to create space/time panels of temporal and spatial features for each unique space/time unit pair. We now engineer a new features: temporal lag, which lets us examine temporal correlation in EMS calls. For instance, lagHour is the number of EMS calls in the past hour. In Figure XX, we see that 15 minute lag appears to correlate with EMS count. 
```{r Time Exploration: panels + time lags}
...
#0.01383557 
table(cor_lag$EMS_Count, cor_lag$lagHour)  
#        1     2     3     4
#  1 14252  1127    80     6
#  2  1136   107     3     0
#  3    73    10     1     0
...
study.panel.net <- left_join(study.panel, final_net, by= "uniqueID") %>%
  na.omit() %>%
  mutate(tract = as.factor(tract))

#test corelation
numericVars <-
  select_if(st_drop_geometry(study.panel.net), is.numeric) %>% na.omit()
...
reg1_time <- 
  lm(EMS_Count ~  hour(interval60) + dotw + Temperature + Precipitation + visibility + wind_gust + Wind_Speed, data = EMSallTrain)

reg2_space <- 
  lm(EMS_Count ~ uniqueID + Violent_Crime + bar.nn + downtown.nn, data = EMSallTrain)
  
reg3_time_space <- 
  lm(EMS_Count ~ hour(interval60) + dotw + Temperature + Precipitation + visibility + wind_gust + Wind_Speed + uniqueID + Violent_Crime + bar.nn + downtown.nn, data = EMSallTrain)

reg4_lag <-   lm(EMS_Count ~ hour(interval60) + dotw + Temperature + Precipitation + visibility + wind_gust + Wind_Speed + uniqueID + Violent_Crime + bar.nn + downtown.nn + lagHour + lag2Hours + lag4Hours + lag12Hours + lag1day, data = EMSallTrain)
...
week_predictions <- 
  EMS.Test.weekNest %>% 
    mutate(Time = map(.x = data, fit = reg1_time, .f = model_pred),
           Space = map(.x = data, fit = reg1_time, .f = model_pred),
           Time_Space = map(.x = data, fit = reg1_time, .f = model_pred),
           Time_Space_Lag = map(.x = data, fit = reg1_time, .f = model_pred),) %>%
...
week_predictions %>%
  ...
  ggplot(aes(Week, MAE)) + 
    geom_bar(aes(fill = Regression, colour = Regression), position = "dodge", stat="identity") +
...
#BC: can't fix this, I can't open week_predictions
week_predictions.rd <- left_join(week_predictions, EMS_18_20, by="interval60")
week_predictions <- week_predictions.rd %>% 
    mutate(interval60 = map(data, pull, interval60),
           reporting_district = map(data, pull, reporting_district)) %>%
    dplyr::select(interval60, reporting_district, Observed, Prediction, Regression, Absolute_Error, sd_AE) %>%
    unnest(cols = c(interval60, reporting_district, Observed, Prediction))

week_predictions.twocol <-week_predictions %>%
...
week_predictions <- left_join(week_predictions, week_predictions.twocol, by="reporting_district")

week_predictions.latlon <- left_join(week_predictions, EMS_2020[,c("reporting_district", "latitude", "longitude", "dotw", "week", "interval60")],by = "reporting_district")

ggplot()+
  geom_sf(data = (SM_Base%>%
    st_transform(st_crs(fishnet))), color = "grey", fill = "transparent")+
  geom_point(data=week_predictions.latlon, aes(x = longitude, y = latitude, color = MAE), 
