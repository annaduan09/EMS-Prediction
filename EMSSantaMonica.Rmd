...
What about the temporal risk factors for EMS incidents?
```{r exploratory}
...
covid <- st_read("/Users/annaduan/Documents/GitHub/EMS-Prediction/coronavirus.csv") %>%
  filter(place == "Santa Monica") %>%
  st_as_sf(coords = c("x", "y"), crs = 4326, agr = "constant") %>%
    st_transform('EPSG:2225') %>%
    mutate(date = as.POSIXct(date)) %>%
  dplyr::select(date, confirmed_cases, geometry) %>%
...
ggplot(covid, aes(date,Case_Count)) + 
  geom_area(fill = "pink") +
  geom_line(color = "pink") +
...
study.panel.15 <- 
  ...
  summarize(EMS_Count = sum(EMS_Counter, na.rm=T)) %>%
  left_join(weather.Panel.15, by = "interval15") %>%     #AD: weather.Panel doesn't have interval15, do you know how to make it?
  ungroup() %>%
  filter(is.na(reporting_district) == FALSE) %>%
  mutate(week = week(interval15),
         dotw = wday(interval15, label = TRUE))
...
tg   <- as.POSIXct("2019-11-22 01:00:00 UTC")
xmas <- as.POSIXct("2019-12-24 01:00:00 UTC")
mondays <- 
  mutate(study.panel.rd,
  ...
  mutate(corlag.lag1ct.15, Legend = "EMS 15 min lag count")) %>%
    group_by(Legend, interval15) %>% 
      ...
      ggplot(aes(interval15, ems_Count, colour = Legend)) + geom_line() +
        ...
        labs(title="EMS trips by week",
             subtitle="Dotted lines for Thanksgiving", 
             x="Day", y="EMS Count") +
        plotTheme() + theme(panel.grid.major = element_blank())   
```
...
ggplot(EMS_18_20 %>%
  ...
  plotTheme(),
ncol=2,
top = "EMS Calls by Week and Day of Week")
...
study.panel.sample <- sample_n(study.panel, 50000, fac = "lagHour") %>%
  na.omit() %>%
  dplyr::select(lagHour, EMS_Count)

ggplot(data = study.panel.sample, aes(x = lagHour, y=EMS_Count)) +
  geom_point(size=2, shape=20) +
  labs(title = "EMS Call 1 Hour Lag", subtitle = "Santa Monica, California") +
...
reg_temp1 <- 
  lm(EMS_Count ~  reporting_district + hour(interval60) + dotw + Temperature + Precipitation + visibility + wind_gust + Wind_Speed + lagHour + lag2Hours +lag3Hours +lag12Hours + lag4Hours + lag1day, 
     data=EMSallTrain)
...
week_predictions <- 
  EMS.Test.weekNest %>% 
    mutate(Time_Space_FE_timeLags = map(.x = data, fit = reg_temp1, .f = model_pred)) %>%  #AD: not working.
...
```{r spatial bi model}
#AD: don't think we need this anymore 
#set.seed(10)
