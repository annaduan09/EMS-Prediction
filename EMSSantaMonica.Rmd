...
EMS2020.sf <-
  ...
  mutate(CoastDist=(geosphere::dist2Line(p=st_coordinates(st_centroid(EMS2020.sf)),
         ...
         distWater = EMS2020.sf$CoastDist)
...
plotData.lag <-
  filter(as.data.frame(study.panel)) %>%
  dplyr::select(starts_with("lag"), EMS_Count) %>%
...
correlation.lag <-
  group_by(plotData.lag, Variable) %>%
    summarize(correlation = round(cor(Value,EMS_Count, use = "complete.obs"), 2)) 
```
