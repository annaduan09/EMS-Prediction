...
_tractData <-   rbind(violentCrime,trash, streetlight, signandmarking, socialservice, harmful_lu) %>%
  st_join(., fishnet, join=st_within) %>%
  st_drop_geometry() %>%
  group_by(uniqueID, Legend) %>%
  summarize(count = n()) %>%
...
# MAKE  MAPPABLE
.long <- 
  gather(_tractData, Variable, value, -geometry, -uniqueID)
...
_tractData <-
  _tractData %>%
    mutate(
      violentCrime.nn =
        nn_function(st_c(st_coid()), st_c(violentCrime),3),
      trash.nn =
        nn_function(st_c(st_coid()), st_c(trash),3),
      violentCrime.nn =
        nn_function(st_c(st_coid()), st_c(violentCrime),3),
      streetlightissue.nn =
        nn_function(st_c(st_coid()), st_c(streetlight),3),
      signandmarking.nn = 
        nn_function(st_c(st_coid()), st_c(signandmarking),3),
      socialservice.nn = 
        nn_function(st_c(st_coid()), st_c(socialservice),3),
      harmfullu.nn = 
        nn_function(st_c(st_coid()), st_c(harmful_lu),1),
      sport.nn = 
        nn_function(st_c(st_coid()), st_c(st_coid(sport)),1),
      bar.nn =
        nn_function(st_c(st_coid()), st_c(bars),1),
      sport.nn =
        nn_function(st_c(st_coid()), st_c(st_coid(sport)),1),
      downtown.nn =
        nn_function(st_c(st_coid()), st_c(st_coid(downtown)),1),
      intersection.nn = 
        nn_function(st_c(st_coid()), st_c(intersections),3),
      uniqueID = as.numeric(rownames(.)))

.long.nn <- 
  dplyr::select(_tractData, ends_with(".nn")) %>%
    gather(Variable, value, -geometry)

vars <- unique(.long.nn$Variable)
mapList <- list()
...
################Final Net################
final_net <-
  inner_join(EMS_2020_net, st_drop_geometry(_tractData), by="uniqueID")
...
for(i in vars){
  mapList[[i]] <- 
    ggplot() +
      geom_sf(data = filter(.long, Variable == i), aes(fill=value), colour=NA) +
