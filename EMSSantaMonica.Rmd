...
###need to subset here
#AD: actually don't - I figured out why we have missing data and it's because some of the fishnet grids don't have correlating census data. I fixed it though :)
ACSTracts <- 
#####
  
ACS <-
  rbind(
    st_centroid(ACS)[SM_Buffer,] %>%
      st_drop_geometry() %>%
      left_join(ACS) %>%
      st_sf() %>%
      mutate(inSM = "YES"),
    st_centroid(ACS)[SM_Buffer, op = st_disjoint] %>%
      st_drop_geometry() %>%
      left_join(ACS) %>%
      st_sf() %>%
      mutate(inSM = "NO")) %>%
...
ggplot() + #AD - used this to diagnose hte issue with the missing data/geometries
  ...
  geom_sf(data = SM_Base, colour = "green", fill = "transparent") +
  mapTheme()
...
#Map response time since 2010
  ...
  scale_fill_viridis(option = "A", name = "Average time in minutes") +
  scale_colour_viridis(option = "A", name = "Minutes") +
...
<<<<<<< HEAD
>>>>>>> b374581df1c97eef07d910683e4aaf01cd9188a4
=======
>>>>>>> b374581df1c97eef07d910683e4aaf01cd9188a4
...
vars_net.long.select <-      #AD: this is from predictive policing, haven't changed code yet.
   gather(vars_net_noname, Variable, value, -geometry) %>%
  dplyr::filter(Variable == "medHHInc" | Variable == "pctMaleYouth"| Variable == "pctNoHS"| Variable == "pctPoverty" | Variable == "markings" | Variable == "servicerequest" | Variable == "Trash" | Variable == "youthUnemploy" | Variable == "uninsured")
