...
#interesting! The response time in 2020 is actually lower than 2018-2019 on average.
#The EMS is not that overwhelmed 
#AD - Yeah i noticed! I read a study this morning about EMS calls during the pandemic and they found that in some cases people were less likely to call EMS due to fear of catching covid at the hospital. 
``` 

To better understand the spatial distribution of EMS calls, we also analyze the spatial distribution of some of its risk factors. We use health factors related to health emergencies such as past incidence of stroke, heart disease, and high blood pressure. We also use demographic features related to groups at high risk of relying on emergency medical services as their primary healthcare including poverty rate, unemployment, and percent white. In addition, we use environmental features related to poor community health including harmful land use, street light issues, and municipal service requests. Finally, we map violent crime incidents, perhaps the most direct proxy for EMS demand. 
...
Figure XX shows Covid-19 cases in Santa Monica. Daily case counts are relatively low until a peak in July. Cases begin to increase rapidly starting in October. There doesn't appear to be a correlation with EMS calls, however. This may be because Santa Monica has relatively few cases of Covid-19.
```{r covid cases / time}
...
holidayPlot <- rbind(
  mutate(EMS.Train, Legend = "Training"), 
  mutate(EMS.Test, Legend = "Testing")) %>%
...
ggplot(EMS_18_20 %>% 
  ...
  labs(title="EMS Calls Weekend vs Weekday, 2020",
       x="Hour", 
       y="Trip Counts")+
     plotTheme(),
ncol=2,
...
The two hotspots are in Downtown and Mid City. Downtown: beach, Santa Monica pier, muscle beach: likely accidents from physical activity/overall high population density/nightlife
...
trainIndex <- createDataPartition(study.panel.rd$EMS_Count, 
                                  ...
                                  list = FALSE,
                                  times = 1)
EMSallTrain <- study.panel.rd[ trainIndex,]
EMSallTest  <- study.panel.rd[-trainIndex,] 

EMS.Train <- subset(study.panel.rd,
     interval60 >= as.POSIXct('2018-12-08') &
     interval60 < as.POSIXct('2020-4-08')
     )
EMS.Test <- subset(study.panel.rd,
     interval60 >= as.POSIXct('2020-4-08') &
...
EMSallTest <- EMSallTest %>%
  rename(Week = week) #AD: had to rename week as Week because R thought we were referencing the lubridate function week 

reg1_time <- 
  lm(EMS_Count ~  hour(interval60) + dotw + Temperature + Precipitation + visibility + wind_gust + Wind_Speed, data = EMSallTrain)

reg2_space <- 
  #reporting_district + health factors + dotw + temperature 
  
reg3_time_space <- 
  #reportinig district + dtw + hour(interval60) + temperature 

reg4_lag <-
  #lagHour + lag2Hours +lag3Hours +lag12Hours + lag4Hours + lag1day + all of above
...
week_predictions <- 
  EMS.Test.weekNest %>% 
    mutate(Time_Space_FE_timeLags = map(.x = data, fit = reg1_time, .f = model_pred)) %>%
