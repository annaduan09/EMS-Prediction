...
Next, we examine in greater detail when EMS calls themselves occur. Figure XX shows that they are most frequent on weekdays, and that it first peaks at 1am, then at 12:30pm and 3pm respectively on weekdays and weekends. Additionally, cases peak on Monday.
...
# Feature Engineering  
We now engineer a new feature: temporal lag, which lets us examine temporal correlation in EMS calls. For instance, lagHour is the number of EMS calls in the past hour. In Figure XX, we see that 15 minute lag appears to correlate with EMS count. 
```{r Time Exploration: panels + time lags, message=FALSE, warning=FALSE, results='hide'}
