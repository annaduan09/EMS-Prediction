...
#Past 2 years of calls       #AD: I think it would be better for us to just work with the past 2 years instead of 10. We can use 10 to illustrate how reponse time is in general (it has increased in the past two years), but it'd be best to use just 2 years because I couldn't even generate a panel with the 10 years of data 
EMS_18_20 <- subset(EMS_int,
...
#EMS_18_20.sf <-
#  EMS_18_20.sf %>%
#  mutate(CoastDist=(geosphere::dist2Line(p=st_coordinates(st_centroid(EMS_18_20.sf)),
                                        #line=st_coordinates(coastline$osm_lines)[,1:2])*0.00062137)[,1],
#         distWater = EMS_18_20.sf$CoastDist)
...
EMS_18_20_net <- full_join(EMS_18_20_net.count, st_drop_geometry(EMS_18_20_net.response), by = c("uniqueID"))
...
#Map response time since 2010
  ...
  scale_fill_viridis(option = "A", name = "Minutes") +
  scale_colour_viridis(option = "A", name = "Minutes") +
  labs(title = "Average EMS Response Time Joined to Fishnet, 2010-2020", subtitle = "Santa Monica, CA") +
  mapTheme()  
  
#Call density - 2018-2020
  ggplot() + 
  geom_sf(data = ACS, fill = "black") +
  stat_density2d(data = data.frame(st_coordinates(EMS_18_20.sf)), 
  ...
  labs(title = "Density of EMS Incidents, 2018-2020") +
  mapTheme()
  
#EMS call density - 2010-2020
ggplot() + 
  geom_sf(data = ACS, fill = "black") +
  ...
  scale_fill_gradient(low = "yellow", high = "red", 
                      ...
                      labels=c("Minimum","Maximum"), name = "Density") +
  scale_alpha(range = c(0.0, 0.5), guide = FALSE) +
  labs(title = "Density of EMS Incidents, 2010-2020") +
  mapTheme()
...
ggplot(EMS_18_20 %>% mutate(hour = hour(interval60)))+
  ...
  labs(title="EMS Calls in Santa Monica, by day of week, 2018-2020",
       x="Hour", 
       y="EMS Calls")+
     plotTheme()
...
ggplot(EMS_18_20 %>% mutate(hour = hour(interval60)))+
  ...
  labs(title="EMS Calls in Santa Monica by month, 2018-2020",
       x="Month", 
       y="EMS Calls") +
     plotTheme()
...
vars_net_noname <- subset(vars_net_tractData, select = -c(uniqueID)) %>%
...
correlation.long <-
  st_drop_geometry(final_net) %>%
    dplyr::select(-uniqueID, -cvID, -tract) %>%
...
dat_census <- st_join(EMS_18_20,
        ACSTracts,              #AD: should we join tractData instead?
        join=st_intersects,
              left = TRUE) %>%
...
dat_net <- st_join(EMS_18_20, fishnet, join=st_intersects,left = TRUE) %>% #AD: join final net instead?
  mutate(longitude = unlist(map(geometry, 1)),
         latitude = unlist(map(geometry, 2)))%>%
  as.data.frame() 
...
EMS.Train <- subset(study.panel.15,
     interval15 >= as.POSIXct('2018-12-08') &
     interval15 < as.POSIXct('2019-12-08')
     )
EMS.Test <- subset(study.panel.15,
     interval15 >= as.POSIXct('2019-12-08') &
     interval15 <= as.POSIXct('2020-12-08')
     )

reg_tem <- 
  lm(EMS_Count ~  hour(interval15) + dotw + week + day, 
     data=EMS.Train)
